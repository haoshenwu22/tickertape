...
gcloud run deploy tickertape-api --image=$REPO/backend:latest --region=$REGION --platform=managed --allow-unauthenticated --add-cloudsql-instances=$SQL_CONNECTION --vpc-connector=$VPC_CONNECTOR --set-secrets=DJANGO_SECRET_KEY=django-secret-key:latest,DATABASE_URL=database-url:latest,REDIS_URL=redis-url:latest,ANTHROPIC_API_KEY=anthropic-api-key:latest,GMAIL_ADDRESS=gmail-address:latest,GMAIL_APP_PASSWORD=gmail-app-password:latest --set-env-vars=DEBUG=False,EMAIL_BACKEND=django.core.mail.backends.smtp.EmailBackend,ALLOWED_HOSTS=* --memory=512Mi --cpu=1 --min-instances=0 --max-instances=3 --port=8000
